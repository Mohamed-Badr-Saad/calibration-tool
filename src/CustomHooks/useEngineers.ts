...
export const EngineerContext = createContext<EngineerContextType | undefined>(undefined);


export const useEngineers = () => {
  const context = useContext(EngineerContext);
  if (!context) throw new Error("useEngineers must be used within EngineerProvider");
  return context;
};