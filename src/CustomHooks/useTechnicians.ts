...
export const TechnicianContext = createContext<TechnicianContextType | undefined>(undefined);

export const useTechnicians = () => {
  const context = useContext(TechnicianContext);
  if (!context) throw new Error("useTechnicians must be used within TechnicianProvider");
  return context;
};
